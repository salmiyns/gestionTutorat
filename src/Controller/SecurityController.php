...
class SecurityController extends AbstractController
{
    ...
    ): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            */
            ...
            ->from('no-reply@nouvelle-techno.fr')
            ->to($user->getEmail())
            ->text(
                "Bonjour,<br><br>Une demande de réinitialisation de mot de passe a été effectuée pour le site Nouvelle-Techno.fr. Veuillez cliquer sur le lien suivant : " . $url,
                'text/html'
     ...
     */
    public function resetPassword(Request $request, string $token, UserPasswordEncoderInterface $passwordEncoder,UserRepository $usersRepository)
    {
        // On cherche un utilisateur avec le token donné
        $user = $usersRepository->findOneBy(['reset_token' => $token]);
